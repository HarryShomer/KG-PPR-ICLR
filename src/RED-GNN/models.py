...
class RED_GNN_induc(torch.nn.Module):
    ...
    def forward(self, subs, rels, mode='transductive', test_graph=0):
        n = len(subs)

        n_ent = self.loader.n_ent if mode=='transductive' else self.loader.n_ent_ind[test_graph]
        ...
        for i in range(self.n_layer):
            nodes, edges, old_nodes_new_idx = self.loader.get_neighbors(nodes.data.cpu().numpy(), mode, test_graph)
