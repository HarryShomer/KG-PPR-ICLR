...
# from datasets.ind_dataset import Ind_FB15k237, Ind_WN18RR, Ind_NELL, New_Ind_E, New_Ind_ER
from datasets.ind_dataset import New_Ind
...
@click.option('-path', '--path', type=str)  # Path to folder with data
@click.option('-num_test', '--num_test', type=int, default=1)
@click.option('-seed', '--seed', type=int, default=1)  # Random Seed
...
def main(
    ...
    dataset = New_Ind(dataset_name, path, num_test, True)
    ...
    inference_triples_factory = dataset.all_inf_graphs[0]
    ...
    # )
    # Since not used in original code... (see comment on line 192)
    early_stopper = None
    ...
    for i in range(num_test):
        inf_graph = dataset.all_inf_graphs[i]
        inf_test = dataset.all_inf_test[i]

        finetuning_model.inductive_inference = inf_graph
        finetuning_model.resample_rels()

        metric_results = test_evaluator.evaluate(
            model=finetuning_model,
            mapped_triples=inf_test.mapped_triples,
            additional_filter_triples=[inf_graph.mapped_triples],
            use_tqdm=True,
            batch_size=eval_bs,
        )
        print(f"\n\nTest results for graph {i}")
        print(f"  MRR: {metric_results.mean_reciprocal_rank['both']['avg']}")
        print(f"  Hits@10: {metric_results.hits_at_k['both']['avg'][10]}")
