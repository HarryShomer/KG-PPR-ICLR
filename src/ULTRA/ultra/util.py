...
def parse_args():
    ...
    parser.add_argument("-i", "--inf-graph", help="Inference graph ID", type=int, default=0)
    parser.add_argument("-n", "--num-test", help="Num Test Graphs", type=int, default=1)
...
def build_dataset(cfg, num_test=None):
    data_config = copy.deepcopy(cfg.dataset)
    cls = data_config.pop("class")

    if "new" in data_config:
        dataset = datasets.NewSplit(cfg.dataset.root, cls, num_test=num_test)
    else:
