...
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    ...
    for batch in tqdm(test_loader, "Evaluation"):
...
if __name__ == "__main__":
    ...
    if util.get_rank() == 0:
        ...
        logger.warning("Inference Graph: %s" % inf_graph)
        logger.warning(pprint.pformat(cfg))
    
    task_name = cfg.task["name"]
    dataset = util.build_dataset(cfg, num_test)
    device = util.get_device(cfg)
    
    train_data, valid_data, test_data = dataset[0], dataset[1], dataset[2 + inf_graph]
