...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
                 aggregate_func="pna", layer_norm=False, activation="relu", dependent=True, ppr_test=False):
    ...
    def forward(self, input, query, boundary, edge_index, edge_type, size, edge_weight=None):
        ...
        # Add dummy relation
        if self.ppr_test:
            dummy_rel = torch.ones((batch_size, 1, relation.shape[-1]), device=input.device)
            relation = torch.cat((relation, dummy_rel), dim=1)
