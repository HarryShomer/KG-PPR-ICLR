...
train = TrainData(path, args.data_name)
# test_data = 
...
for epoch in pbar:
	...
	# print("---", msg.shape, relation_triplets.shape, sup.shape)
	# print("---", init_emb_ent.shape, init_emb_rel.shape)
	...
	if ((epoch + 1) % valid_epochs) == 0:
		print("Validation")
		my_model.eval()
		val_init_emb_ent, val_init_emb_rel, val_relation_triplets = initialize(train, train.triplets_with_inv, \
																				d_e, d_r, B)

		val_perf = evaluate(my_model, train, epoch, val_init_emb_ent, val_init_emb_rel, val_relation_triplets)

		if val_perf > best_val_perf and not args.no_write:
			best_val_perf = val_perf
			print("Better validation performance. Saving model!")
			torch.save({'model_state_dict': my_model.state_dict(), \
						'optimizer_state_dict': optimizer.state_dict(), \
						'inf_emb_ent': val_init_emb_ent, \
						'inf_emb_rel': val_init_emb_rel}, \
				f"ckpt/{args.exp}/{args.data_name}/{file_format}_best_{args.seed}.ckpt")
