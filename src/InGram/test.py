...
path = args.data_path = os.path.join(file_path, "..", "..", "new_data", args.data_name) 

test = TestNewData(path, data_type = "test", test_graph=args.test_graph)
...
my_model.load_state_dict(torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best_{args.seed}.ckpt")["model_state_dict"])
...
# test_init_emb_ent = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best_{args.seed}.ckpt")["inf_emb_ent"]
# test_init_emb_rel = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best_{args.seed}.ckpt")["inf_emb_rel"]
test_init_emb_ent, test_init_emb_rel, _ = initialize(test, test_msg, d_e, d_r, B)
...
evaluate(my_model, test, args.target_epoch-1, test_init_emb_ent, test_init_emb_rel, test_relation_triplets, True)
