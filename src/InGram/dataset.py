...
class TrainData():
	def __init__(self, path, dataname):
		...
		self.create_filter(path + '/train_graph.txt')
		self.test_triplets = self.read_valid(path + '/valid_samples.txt')
		

	def read_valid(self, path):
		"""
		Read valid triples

		Use saved name2id vars to convert to ix
		"""
		triplets = []
		with open(path, 'r') as f:
			for line in f.readlines():
				h, r, t = line.split()
				triplets.append((self.ent2id[h], self.rel2id[r], self.ent2id[t]))

		return np.array(triplets)


	def create_filter(self, path):
		"""
...
class TestNewData():
	def __init__(self, path, test_graph=0, data_type = "test"):
		self.path = path
		self.data_type = data_type
		self.ent2id = None
		self.rel2id = None
		self.test_graph = test_graph
		self.id2ent, self.id2rel, self.msg_triplets, self.test_triplets, self.filter_dict = self.read_triplet()
		self.num_ent, self.num_rel = len(self.id2ent), len(self.id2rel)
	...
	def read_triplet(self):
		...
		graphfile = f"/test_{self.test_graph}_graph.txt"
		testfile = f"/test_{self.test_graph}_samples.txt"

		with open(self.path + graphfile, 'r') as f:
			for line in f.readlines():
				h, r, t = line.split()
				id2ent.append(h)
				id2ent.append(t)
				...
				msg_triplets_nonid.append((h, r, t))
		...
		with open(self.path + testfile, 'r') as f:
			for line in f.readlines():
				h, r, t = line.split()
		...
		filter_dict = defaultdict(list)
		for triplet in msg_triplets_nonid:
			h,r,t = triplet
			if ('_', self.rel2id[r], self.ent2id[t]) not in filter_dict:
				filter_dict[('_', self.rel2id[r], self.ent2id[t])] = [self.ent2id[h]]
			else:
