...
def evaluate(my_model, target, epoch, init_emb_ent, init_emb_rel, relation_triplets, is_test=False):
    with torch.no_grad():
        my_model.eval()
        if is_test:
            msg = torch.tensor(target.msg_triplets).cuda()
        else:
            msg = torch.tensor(target.triplets_with_inv).cuda()

        sup = torch.tensor(target.test_triplets).cuda()
